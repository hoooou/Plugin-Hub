 ...
 * 提速配置：默认下载 2 秒/节点（100MB 文件）；国外出口检测固定开启且与下载并行（耗时被下载吸收）；预检失败不重试；预检并发上限 60。 */
...
const onRun = async () => {
  ...
  const content = {
    template: `
      <div class="p-8 text-12">
        <div class="grid grid-cols-2 gap-8">
          ...
          <div><div class="text-gray-500 mb-4">节点过滤（名称关键词，留空=全部）</div><Input v-model="form.filter" :disabled="running" placeholder="如 US 美国 LAX，空格分隔多个" /></div>
    ...
    setup() {
      ...
      const form = ref({
        group: Plugin.GroupName && initial.some((p) => p.name === Plugin.GroupName || p.tag === Plugin.GroupName) ? Plugin.GroupName : (initial[0]?.name || initial[0]?.tag || ''),
        url: Plugin.TestUrl || 'https://hkg.download.datapacket.com/100mb.bin',
        presetUrl: '',
        pingUrl: Plugin.PingUrl || 'https://www.gstatic.com/generate_204',
        maxDelay: Number(Plugin.MaxDelayMs) || 3000,
        seconds: Number(Plugin.TimeoutSeconds) || 2,
      ...
      /** 将一条成功测速结果对应的节点切换为各策略组的选择：先校验节点仍存在，再弹出面板让用户勾选要切换的策略组；不做任何测速恢复。 */
      const useResult = async (row) => {
        ...
        /* 找出所有包含该节点的 Selector 组，供用户勾选切换目标 */
      ...
      const deleteHistory = async () => {
        ...
        try { await Plugins.RemoveFile(historyFilePath()) } catch {}
      }
        ...
        try {
          for (let i = 0; i < rows.length; i++) {
            ...
            if (!proxy || !group.all.includes(row.name)) {
              row.overseasIp = ''; row.overseasPlace = ''; row.overseasSource = ''
      ...
      onMounted(async () => { history.value = await loadHistory() })

      /** 请求停止当前两阶段测试：取消预检请求或当前下载，之后不再启动新任务。 */
      const stop = () => { runBatchTest.stop?.() }
      /** 清除当前面板中的测速结果。 */
      const clear = () => { if (!running.value) results.value = [] }
      ...
      const start = async () => {
        ...
        runBatchTest.active = runBatchTest({
          ...
          onPrecheck: (done, count, etaSec) => {
            ...
            /* 预检全部完成时立即按延迟升序重排表格：下载阶段也按此顺序推进，表格顺序与下载顺序一致 */
            if (done >= count && results.value.length) results.value = [...results.value].sort(compareDelayRows)
          },
          ...
          if (rows.length) {
            ...
            const payload = { schema: 'batch-download-speed', version: 2, savedAt: new Date().toISOString(), groupName: form.value.group, testUrl: String(form.value.url).trim(), results: rows }
            saveHistory(payload)
      ...
      return { form, results, history, running, progress, total, currentNode, statusText, groupOptions, presetUrlOptions, progressPercent, formatTime, formatDelay, overseasText, start, stop, clear, useResult, deleteHistory, egressRunning, runEgress, stopEgress }
    }
  }
...
/** 预检完成后的临时排序：延迟升序（无延迟的失败行排最后），使表格顺序与下载顺序一致，便于观察进度。
 * 仅在预检完成时触发一次；下载阶段 upsert 原地替换不破坏顺序；全部测完后仍由 compareResultRows 按速度重排。 */
const compareDelayRows = (a, b) => {
...
/** 从策略组中收集可测速节点：去重、排除 DIRECT/REJECT 与嵌套策略组，按名称关键词过滤（多个关键词空格/逗号分隔，任一匹配），limit 作用于过滤后列表。 */
...
const OVERSEAS_SOURCES = [
  ['cmliussss API', 'https://api.cmliussss.net/api/ipinfo', (body) => { if (!body?.ip) throw new Error('返回格式异常'); return { ip: body.ip, place: `${body.country_code || ''} AS${body.asn || ''} ${body.as_name || ''}` } }],
  ['ipinfo.io', 'https://ipinfo.io/json', (body) => { if (!body?.ip) throw new Error('返回格式异常'); return { ip: body.ip, place: `${body.country || ''} ${body.org || ''}` } }],
  ['ipapi.co', 'https://ipapi.co/json/', (body) => { if (!body?.ip) throw new Error('返回格式异常'); return { ip: body.ip, place: `${body.country_code || body.country_name || ''} ${body.org || (body.asn ? `AS${body.asn}` : '')}` } }]
]

/** 内置的下载测速预设地址，可在面板中一键选择。 */
const PRESET_URLS = [
  { label: '🇭🇰 香港', value: 'https://hkg.download.datapacket.com/100mb.bin' },
  { label: '🇸🇬 新加坡', value: 'https://sgp.download.datapacket.com/100mb.bin' },
  { label: '🇯🇵 日本', value: 'https://tyo.download.datapacket.com/100mb.bin' },
  { label: '🇺🇸 美西', value: 'https://lax.download.datapacket.com/100mb.bin' },
  { label: '🇺🇸 美东', value: 'https://ash.download.datapacket.com/100mb.bin' }
]
...
const runBatchTest = async ({ api, groupName, url, pingUrl, maxDelay, seconds, concurrency, limit, downloadLimit, filter, onTotal, onPrecheck, onNode, onDownload, onResult }) => {
  ...
  /* 下载/出口检测流量会被「下载测速分流」规则路由到「测速专用」组：
   * 测速期间同步把该组切到当前被测节点，保证测的确实是这个节点；结束后恢复。 */
  const speedGroup = Object.values(api.proxies || {}).find((g) => g && g.type === 'Selector' && Array.isArray(g.all) && /测速/.test(String(g.name || g.tag || '')) && (g.name || g.tag) !== groupName)
  const speedOriginal = speedGroup ? speedGroup.now : null
  ...
  try {
    ...
    for (; downloadIndex < downloadTarget; downloadIndex++) {
      ...
      try {
        ...
        /* 下载测速与国外出口检测并行：两者都经同一本地代理入口走当前切换的节点，互不干扰，出口检测耗时被下载时长吸收。 */
      ...
      downloaded += 1
      onResult(row)
      /* 估算剩余下载耗时：每节点约 = 下载时长 + 节点切换开销（出口检测与下载并行，不额外计） */
